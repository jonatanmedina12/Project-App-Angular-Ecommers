...
export class InventoryComponent implements OnInit {
  ...
  onSubmit() {
    if (this.inventarioForm.valid) {
      const inventarioDTO: InventarioDTO = this.inventarioForm.value;
      if (this.editando) {
        this.inventarioService.actualizarInventario(inventarioDTO.productoId, inventarioDTO).subscribe(
          () => {
        ...
        this.inventarioService.crearInventario(inventarioDTO).subscribe(
          () => {
            this.mostrarMensaje('Inventario creado con éxito');
            this.cargarInventarios();
            this.resetForm();
          },
          error => this.mostrarError('Error al crear inventario')
        );
      }
    }
  }
  ...
  actualizarStock(productoId: number, cantidad: number) {
    this.inventarioService.actualizarStock(productoId, cantidad).subscribe(
      (inventarioActualizado) => {
        const index = this.inventarios.findIndex(inv => inv.productoId === productoId);
        if (index !== -1) {
